...
WITH TEMP AS(
SELECT
	count(CASE WHEN FO.FLG_GROSS_ORDER = 1 THEN FO.FLG_GROSS_ORDER ELSE NULL END) AS TOT_DELIVERED,
	C.DSC_COUNTRY
FROM 
	DW.FCT_ORDERS as FO
	INNER JOIN _AUX.COUNTRIES AS C ON C.COD_COUNTRY = FO.COD_COUNTRY
WHERE
	FO.FLG_GROSS_ORDER = 1 AND FO.SK_DT_ORDER_CREATION BETWEEN 20200101 AND 20221231
GROUP BY
	C.DSC_COUNTRY
),
TEMP1 AS(
SELECT
	count (CASE WHEN FO.MTR_GROSS_ORDERS = 1 THEN FO.MTR_GROSS_ORDERS ELSE NULL END) AS TOT_GROSS,
	C.DSC_COUNTRY
FROM
	DW.FCT_ORDERS as FO
	INNER JOIN _AUX.COUNTRIES AS C ON C.COD_COUNTRY = FO.COD_COUNTRY
WHERE
	FO.MTR_GROSS_ORDERS = 1 AND FO.SK_DT_ORDER_CREATION BETWEEN 20200101 AND 20221231 
GROUP BY
	C.DSC_COUNTRY
)
SELECT	
	T.DSC_COUNTRY AS COUNTRY,
	T.TOT_DELIVERED,
	T1.TOT_GROSS,
	round (((T.TOT_DELIVERED - T1.TOT_GROSS) * 1.0 / T.TOT_DELIVERED) * 100,2)  AS TOT_PERC_NOT_DELIVERED
FROM
	TEMP AS T
